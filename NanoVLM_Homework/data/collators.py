    ...
    def __call__(self, batch):
        ...
        for i in range(len(batch)):
            ...
            # The tokenizer left-pads short sequences, so find where non-padding tokens begin (using attention_mask)
                ...
                first_token_pos = attention_mask[i].nonzero()[0].item()  # the first 1 (non-zero value) in the attention mask for a given batch sample marks the first non-padding token
